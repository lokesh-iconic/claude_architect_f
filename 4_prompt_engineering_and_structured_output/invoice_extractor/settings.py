...
Same contract as modules 1 and 2: `--mode mock` is always honoured, `auto`
...
class Settings:
    ...
    batch_poll_timeout_s: float = 1800.0
    batch_max_rounds: int = 3
...
def load_settings(
    ...
    *,
    ...
    base = Settings(
        ...
        api_key=key or None,
        model=os.getenv("IE_MODEL", "claude-opus-5"),
        effort=os.getenv("IE_EFFORT", "medium"),
        confidence_threshold=_env_float("IE_CONFIDENCE_THRESHOLD", 0.8),
        max_attempts=_env_int("IE_MAX_ATTEMPTS", 3),
        request_timeout_s=_env_float("IE_REQUEST_TIMEOUT", 120.0),
        refusal_fallback=_env_flag("IE_REFUSAL_FALLBACK", True),
        batch_poll_interval_s=_env_float("IE_BATCH_POLL_INTERVAL", 30.0),
        batch_poll_timeout_s=_env_float("IE_BATCH_POLL_TIMEOUT", 1800.0),
        batch_max_rounds=_env_int("IE_BATCH_MAX_ROUNDS", 3),
    )
    ...
    if not key:
        if requested_mode == "live":
            raise SystemExit(
                ...
                f"Set it in {dotenv_path}, or drop --mode live to run the mock backend."
            )
