...
def load_settings(
    ...
    *,
    ...
    base = Settings(
        ...
        api_key=key or None,
        model=os.getenv("RA_MODEL", "claude-opus-5"),
        effort=os.getenv("RA_EFFORT", "medium"),
        request_timeout_s=_env_float("RA_REQUEST_TIMEOUT", 120.0),
        refusal_fallback=_env_flag("RA_REFUSAL_FALLBACK", True),
        tool_timeout_s=_env_float("RA_TOOL_TIMEOUT", 3.0),
        keep_recent_turns=_env_int("RA_KEEP_RECENT_TURNS", 4),
        max_tool_rounds=_env_int("RA_MAX_TOOL_ROUNDS", 8),
        max_action_attempts=_env_int("RA_MAX_ACTION_ATTEMPTS", 3),
    )
