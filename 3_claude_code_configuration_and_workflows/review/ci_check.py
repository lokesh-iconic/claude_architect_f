...
(review/claude_cli.py) actually uses `-p` -- Claude Code's non-interactive
print mode -- with `--output-format json`. Also confirms the posting side
(review/github_client.py) targets the PR review-comments API, which is what
...
CLAUDE_CLI_PATH = Path(__file__).resolve().parent / "claude_cli.py"
GITHUB_CLIENT_PATH = Path(__file__).resolve().parent / "github_client.py"
